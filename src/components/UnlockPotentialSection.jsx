...
const containerVariants = {
  ...
  visible: {
    opacity: 1,
    transition: {
      staggerChildren: 0.2, // Time between each item fading in
    },
  },
};
  ...
  <div className="relative w-16 h-16 flex items-center justify-center mb-10">
    <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100">
      <circle
        ...
        stroke="currentColor"
        strokeWidth="1"
        strokeDasharray="2 8" // Creates the sparse dotted effect
        strokeLinecap="round"
...
const UnlockPotentialSection = () => {
  return (
    <section className="w-full bg-white text-black py-24 md:py-32 lg:py-48 px-6 md:px-12 lg:px-24 flex justify-center overflow-hidden">
      <motion.div
        variants={containerVariants}
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true, margin: "-15%" }}
        className="w-full max-w-[1400px] grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-20 lg:gap-y-32 gap-x-12 lg:gap-x-16"
      >
        {/* =========================================
            ROW 1
        ========================================= */}
        ...
        >
          <h2 className="text-[clamp(3.5rem,5vw,6rem)] tracking-tight leading-[1] font-medium">
            Unlock Your <br />
            Project's <span className="italic font-serif font-light">
              Full
            </span>{" "}
            <br />
